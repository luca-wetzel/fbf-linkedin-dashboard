...
export async function GET(req: NextRequest, { params }: { params: { slug: string } }) {
  ...
  const { data: orgIcpData } = await getServiceSupabase()
  ...
  const [postsRes, followersRes, icpRes, goalsRes] = await Promise.all([
    getServiceSupabase().from('li_posts').select('*').in('member_id', memberIds),
    getServiceSupabase().from('li_follower_history').select('*').in('member_id', memberIds),
    getServiceSupabase().from('li_icp_signals').select('*').in('member_id', memberIds),
    getServiceSupabase().from('li_goals').select('*').in('member_id', memberIds),
  ])
